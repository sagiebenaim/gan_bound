    ...
    def get_fm_loss(self, real_feats, fake_feats):
        ...
        for real_feat, fake_feat in zip(real_feats, fake_feats):
            ...
            if self.cuda:
                ...
                loss = self.feat_criterion(l2, Variable(torch.ones(l2.size())))
            losses += loss
    ...
    def run(self):
        ...
        for epoch in range(self.args.epoch_size):
            ...
            for i in range(self.n_batches):
                ...
                self.gan_loss_A = (self.gen_loss_B * 0.1 + self.fm_loss_B * 0.9)
                self.gan_loss_B = (self.gen_loss_A * 0.1 + self.fm_loss_A * 0.9)
                self.gen_loss_A_total = self.gan_loss_B * (1. - rate) + self.recon_loss_A * rate
                self.gen_loss_B_total = self.gan_loss_A * (1. - rate) + self.recon_loss_B * rate
